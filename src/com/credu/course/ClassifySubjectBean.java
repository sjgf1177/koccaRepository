...
public class ClassifySubjectBean {
	...
	public int InsertSubjectClassification(RequestBox box) throws Exception {
		...
		try {
			...
			sql =  "insert into TZ_SUBJATT(subjclass,upperclass,middleclass,lowerclass,classname,luserid,ldate) ";
	...
	public int UpdateSubjectClassification(RequestBox box) throws Exception {
		...
		try {
			...
			sql = "update TZ_SUBJATT set classname = ?, luserid = ?, ldate = to_char(sysdate,'YYYYMMDDHH24MISS') ";
			sql+= " where subjclass = ? ";
	...
	public int DeleteSubjectClassification(RequestBox box) throws Exception {
		...
		try {
			...
			if (v_middleclass.equals("000")) {
				sql = "select upperclass from tz_subjatt where upperclass = '" + v_upperclass + "' and middleclass!='000'";
				ls = connMgr.executeQuery(sql);
				...
				//중분류인 경우 과정을 확인
				sql = "select subjclass from tz_subj where subjclass = '" + v_subjclass + "'";
				ls = connMgr.executeQuery(sql);
			...
			if (isOk == 1) {
				...
				sql = "delete from TZ_SUBJATT where subjclass = ? ";
	...
	public static ArrayList<DataBox> getUpperClassList(RequestBox box) throws Exception {
		...
		try {
			...
			sql+= "    where middleclass = '000'  ";
			sql+= "      and lowerclass  = '000'  ";
			//임시 필터링(개발완료 후 보면 삭제요망
			sql+= "      and upperclass not in ('B01', 'K01', 'G01') ";
			sql+= " order by upperclass, middleclass ";
	...
	/**
	온라인과정 대분류 리스트[페지, 경영직무 리스트에서 제외]
	@param box          receive from the form object and session
	@return ArrayList   과정분류코드 리스트
	 */
	public static ArrayList<DataBox> getUpperClassList2(RequestBox box) throws Exception {
		DBConnectionManager connMgr = null;
		ListSet ls = null;
		ArrayList<DataBox> codeList = null;
		String sql  = "";
		...
		try {
			...
			//임시 필터링(개발완료 후 보면 삭제요망
			sql+= "      and upperclass not in ('B01', 'K01', 'G01') ";
			sql+= "      and a.upperclass not in ('X01', 'W01')";
			sql+= " order by upperclass, middleclass ";
