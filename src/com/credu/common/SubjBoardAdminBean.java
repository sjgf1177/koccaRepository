...
public class SubjBoardAdminBean {
	...
	public ArrayList selectBoardList(RequestBox box) throws Exception {
		...
		try {
			if (!box.getString("s_grcode").equals("")) {
				...
				head_sql  = " select a.TABSEQ, (SELECT SUBJNM FROM TZ_SUBJ P1, TZ_BDS P2 WHERE TABSEQ = A.TABSEQ AND P1.SUBJ = P2.SUBJ) SUBJNM, a.seq seq, a.userid userid, a.name name, a.title title, count(b.realfile) upfilecnt,\n";
				head_sql += "        a.indate indate, a.cnt cnt, a.refseq refseq, a.levels levels, a.position position\n";
				body_sql += "   FROM TZ_BOARD a, TZ_BOARDFILE b, TZ_BDS C, TZ_SUBJSEQ D\n";
				body_sql += "  where a.tabseq  =  b.tabseq(+)\n";
				body_sql += "    and a.seq     =  b.seq(+)\n";
				body_sql += "    AND A.TABSEQ = C.TABSEQ   \n";
				body_sql += "    AND C.TYPE='SB'           \n";
				body_sql += "    AND C.SUBJ = D.SUBJ       \n";
				body_sql += "    AND C.SUBJSEQ = D.SUBJSEQ \n";
				body_sql += "    AND C.YEAR = D.YEAR       \n";
				if (box.getString("s_subjcourse").length()>0) body_sql += " AND D.SUBJ LIKE DECODE(NVL('"+ box.getString("s_subjcourse") +"', 'ALL'), 'ALL', '%', '"+ box.getString("s_subjcourse") +"')\n";
				if (box.getString("s_gyear").length()>0){
					if(!box.getString("s_gyear").equals("ALL") || !box.getString("s_gyear").equals("")){
						//body_sql += " AND D.YEAR LIKE DECODE(NVL('"+ box.getString("s_gyear") +"', '%'), 'ALL', '%', '"+ box.getString("s_gyear") +"')\n";
						body_sql += " AND D.YEAR = '"+box.getString("s_gyear")+"' \n";
					}
				}
				if (box.getString("s_grseq").length()>0){
					if(!box.getString("s_grseq").equals("ALL") || !box.getString("s_grseq").equals("")){
						//body_sql += " AND D.GRSEQ LIKE DECODE(NVL('"+ box.getString("s_grseq") +"', '%'), 'ALL', '%', '"+ box.getString("s_grseq") +"')\n";
						body_sql += " AND D.GRSEQ = '"+box.getString("s_grseq")+"' \n";
					}
				}
				...
				group_sql += " group by A.TABSEQ, a.seq, a.userid, a.name, a.title, a.indate, a.cnt, a.refseq, a.levels, a.position\n";
				if (box.get("p_orderColumn").length()>0) {
					order_sql += " order by " + box.get("p_orderColumn") + " " + box.get("p_orderType") + ",a.indate desc, a.refseq desc,a.tabseq, position asc\n";
				}
				else
					order_sql += " order by a.refseq desc,a.tabseq, position asc\n";
	...
	public DataBox selectBoard(RequestBox box) throws Exception {
		...
		try {
			...
			sql  = " select D.SUBJNM, D.EDUSTART, D.EDUEND, a.seq seq, a.userid userid, a.name name, a.title title, b.fileseq fileseq, b.realfile realfile, a.content content, \n";
			...
			sql += " from TZ_BOARD a, TZ_BOARDFILE b, TZ_BDS C, TZ_SUBJSEQ D \n";
			sql += "  where a.tabseq  =  b.tabseq(+) \n";
			sql += "    and a.seq     =  b.seq(+) \n";
			sql += "    and a.TABseq     =  C.TABseq \n";
			sql += "    and C.SUBJ = D.SUBJ AND C.SUBJSEQ = D.SUBJSEQ AND C.TYPE = 'SB' AND C.YEAR = D.YEAR \n";
	...
	public int insertBoard(RequestBox box) throws Exception {
		...
		try {
			...
			//----------------------   게시판 table 에 입력  --------------------------
			int index = 1;            sql1 =  " insert into TZ_BOARD(tabseq, seq, userid, name, indate, title, content, cnt, refseq, levels, position, luserid, ldate)  ";
	...
	public int updateBoard(RequestBox box) throws Exception {
		...
		int isOk1 = 1, isOk2=1, isOk3=1;//, isOk4=1;
		int    v_tabseq    = box.getInt("p_tabseq");
		int    v_seq       = box.getInt("p_seq");
	...
	public int insertReply(RequestBox box) throws Exception {
		...
		try {
			...
			if(connMgr != null) { try { connMgr.freeConnection(); }catch (Exception e1) {} }
	 ...
	 * @throws Exception
	 */
	public ArrayList selectBoardReplyList(RequestBox box) throws Exception {
