    ...
    */
    public ArrayList selectPdsList(RequestBox box,String v_type) throws Exception {
        ...
        try {
			...
			// 2005.11.10_하경태 : jdbc 에러로로 인해 쿼리 변경 .
			head_sql =  " select a.seq, a.userid, a.name, a.title, a.indate, a.cnt,a.okyn1,a.okuserid1,a.okdate1,a.okyn2,a.okuserid2,a.okdate2,";
            //sql += "(select count(realfile) filecnt from tz_boardfile where tabseq = a.tabseq and seq = a.seq) filecnt, ";
			head_sql += " (select count(realfile) from tz_boardfile br, tz_board a where br.tabseq = a.tabseq and br.seq = a.seq ) filecnt, ";
			head_sql += " c.cpnm ";
   ...
   */
   ...
   public DataBox selectPds(RequestBox box) throws Exception {
        ...
        try {
            ...
            ls.close();
            //------------------------------------------------------------------------------------                                                     
            sql = "select a.seq, a.kind, a.userid, a.name, a.title, a.content, a.indate, a.cnt,a.okyn1, a.okuserid1, a.okyn2, a.okuserid2, a.okdate1, a.okdate2,b.fileseq, b.realfile, b.savefile, ";
    ...
    */
     public int insertPds(RequestBox box) throws Exception {
        ...
        try {
           ...
           //////////////////////////////////   게시판 table 에 입력  ///////////////////////////////////////////////////////////////////
            sql1 =  "insert into TZ_CPBOARD(TABSEQ,SEQ,TITLE,USERID,NAME,CONTENT,INDATE,REFSEQ,LEVELS,POSITION,CPSEQ, KIND)";
...
//          sql1 += " values (?, ?, ?, ?, ?, empty_clob(), to_char(sysdate, 'YYYYMMDDHH24MISS'), ?, 1, 1, ?, ?)";
          sql1 += " values (?, ?, ?, ?, ?, ?, to_char(sysdate, 'YYYYMMDDHH24MISS'), ?, 1, 1, ?, ?)";
			...
			//v_cpseq    = (String)em.nextElement();
               //System.out.println("v_cpseq:::11111111111111111111111111");
	 //----------------------   게시판 번호 가져온다 ----------------------------
	            sql = "select NVL(max(to_number(kind)), 0) from TZ_CPBOARD";
    ...
    */
    public int deletePds(RequestBox box) throws Exception {
        ...
        try {
            ...
            ls.close();
            //------------------------------------------------------------------------------------
            sql1 = "delete from TZ_CPBOARD where tabseq = " + v_tabseq + " and seq = ? and kind = ?";
