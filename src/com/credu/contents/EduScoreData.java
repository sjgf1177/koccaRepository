...
public class EduScoreData {
	...
	private double gradexam		=0;     //이수기준 중간평가점수
	private double gradhtest    =0;		//이수기준 형성평가점수
	private double gradftest    =0;     //이수기준 최종평가점수
	private double gradreport   =0;     //이수기준 레포트점수
	private double gradetc1	    =0;     //이수기준 기타 참여도(토론)
	...
	public void makeSub(String datatype, double weight, double score, double avscore, double gradscore){
		...
		EduScoreDataSub ds = new EduScoreDataSub(datatype, weight, score, avscore, gradscore);
		eduScoreList.put(String.valueOf(i), ds);
	}
	public void makeScoreList(){

		if(wstep  >0)	makeSub("STEP", wstep, tstep, avtstep, gradstep);
		if(whtest >0) 	makeSub("HTEST", whtest, htest, avhtest, gradhtest);
		if(wmtest >0)	makeSub("MTEST", wmtest, mtest , avmtest, gradexam);
		if(wftest >0)	makeSub("FTEST", wftest, ftest , avftest, gradftest );
		if(wreport>0)	makeSub("REPORT", wreport, report, avreport, gradreport);
		if(wact   >0)	makeSub("ACT", wact, act , avact, 0);
		if(wetc1  >0)	makeSub("ETC1", wetc1, etc1  , avetc1, gradetc1  );
	...
	public int getReviewdays() {
		return Integer.parseInt(reviewdays);
	}
	 ...
	 * @param d
	 */
	public void setGradetc1(double d) {
		gradetc1 = d;
	}
