//**********************************************************
//1. 제      목: SO진단 문제 관리
//2. 프로그램명: SoExamDiagnosBean.java
//3. 개      요:
//4. 환      경: JDK 1.3
//5. 버      젼: 0.1
//6. 작      성: 정은년 2005.8.22
 ...
 */
public class SoExamDiagnosBean {


    public SoExamDiagnosBean() {}
    ...
    */
    public ArrayList selectDiagnosList(RequestBox box) throws Exception {
        ...
        try {
            if (v_action.equals("go")) {
                ...
                list = getDiagnosList(connMgr, p_distcode);
            } else {
                list = new ArrayList();
            }
        }
    ...
    @return ArrayList   평가문제 리스트
    */
    public ArrayList getDiagnosList(DBConnectionManager connMgr, String p_distcode) throws Exception {
        ...
        try {
            sql = " select a.distcode, a.sodiagnosnum, a.sodiagnostype, a.sodiagnostext,    a.soexptext,     ";
    ...
    */
    public ArrayList getSoExamData(DBConnectionManager connMgr, int p_sonum) throws Exception {
        ...
        try {
            sql = "select a.sodiagnosnum,  a.sodiagnostype, ";
            sql+= "       a.sodiagnostext, a.soexptext, a.levels,      a.selcount,  a.saveimage,   a.saveaudio, ";
            ...
            sql+= "  from tz_sodiagnos      a, ";
            sql+= "       tz_sodiagnossel   b, ";
            sql+= "       tz_code      c, ";
            sql+= "       tz_code      d  ";
			// 수정일 : 05.11.04 수정자 : 이나연 _(+)  수정 
//          sql+= " where a.sodiagnosnum  = b.sodiagnosnum(+) ";
	        sql+= " where a.sodiagnosnum  =  b.sodiagnosnum(+) ";
            sql+= "   and a.sodiagnostype = c.code ";
            sql+= "   and a.levels     = d.code ";
            sql+= "   and a.sodiagnosnum  = " + SQLString.Format(p_sonum);
            sql+= "   and c.gubun      = " + SQLString.Format(SoBean.EXAM_TYPE);
            sql+= "   and d.gubun      = " + SQLString.Format(SoBean.EXAM_LEVEL);
            sql+= " order by a.sodiagnosnum, b.selnum ";
...
Log.info.println("sql>>>"+sql);
    ...
    @return int
    */
    public int insertSoExamDiagnos(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        ListSet ls = null;
        ...
        try {
            ...
            // 문제번호 구하기
            sql = " select NVL(max(sodiagnosnum),0)+1 from TZ_SOdiagnos ";
            ls = connMgr.executeQuery(sql);
            if (ls.next()) {
            ...
            // 평가보기 등록
            sql =  " insert into TZ_SOdiagnosSEL(sodiagnosnum, selnum, seltext, isanswer, luserid, ldate) ";
            sql+=  " values (?, ?, ?, ?, ?, ?)";
    ...
    */
    public int insertTZ_SOEXAM(DBConnectionManager connMgr, int p_soexamnum, String  p_sotype, String  p_sotext,     String  p_exptext,   String  p_levels,   int p_selcount, String  p_saveimage,   String  p_saveaudio, 
        ...
        try {
            ...
            sql =  " insert into TZ_SOdiagnos ";
            sql+=  "            ( sodiagnosnum ,sodiagnostype,sodiagnostext,soexptext ,levels    , ";
    ...
    */
    ...
    public int getSonumSeq(String p_subj) throws Exception {
        ...
        maxdata.put("seqtable","tz_sodiagnos");
    ...
    @return int  
    */
    public int updateDiagnos(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        ...
        try {
            ...
            isOk = updateTZ_so(connMgr,   v_subj,     v_sonum,  v_lesson,   v_sotype,    v_sotext,     v_exptext,    v_levels,  v_selcount,   v_saveimage,   v_saveaudio, v_savemovie,   v_saveflash,   v_realimage,   v_realaudio, v_realmovie,   v_realflash, v_luserid );

            isOk = deleteTZ_sosel(connMgr, v_subj, v_sonum);

            //sql =  "insert into TZ_sodiagnosSEL(subj, sodiagnosnum, selnum, seltext, isanswer, luserid, ldate) ";
            //sql+=  " values (?, ?, ?, ?, ?, ?, ?)";
            //pstmt = connMgr.prepareStatement(sql);
            ...
            if (v_sotype.equals("3")) sulcnt = 2; // OX식 
            ...
            for (int i=1; i<=sulcnt; i++) {
                ...
                if (!v_seltext.trim().equals("")) {
                    v_selnum++;
                    v_isanswer = box.getString("p_isanswer"+String.valueOf(i));
                    //isOk = insertTZ_SOEXAMSEL(pstmt, v_subj,  v_sonum, v_selnum, v_seltext, v_isanswer, v_luserid);
                    isOk = insertTZ_SOEXAMSEL(pstmt, v_sonum, v_selnum, v_seltext, v_isanswer, v_luserid);
                }
            }
     ...
     public int deleteDiagnos(RequestBox box) throws Exception {
          DBConnectionManager connMgr = null;
          int isOk = 0;
          ...
          try {
              ...
              isOk = deleteTZ_so(connMgr, v_subj, v_sonum, v_duserid);
              isOk = deleteTZ_sosel(connMgr, v_subj, v_sonum);
          }
          catch(Exception ex) {
              isOk = 0;
    ...
    public int updateTZ_so(DBConnectionManager connMgr,
                            String  p_subj,     int     p_sonum,  String  p_lesson,   String  p_sotype,
    ...
    public int deleteTZ_so(DBConnectionManager connMgr, String p_subj, int p_sonum, String p_duserid) throws Exception {
        PreparedStatement pstmt = null;
        String sql = "";
        int isOk = 0;
    ...
    public int deleteTZ_sosel(DBConnectionManager connMgr, String p_subj, int p_sonum) throws Exception {
        PreparedStatement pstmt = null;
        String sql = "";
        int isOk = 0;
        ...
        try {
            ...
            sql =  " delete from TZ_sodiagnosSEL ";
            sql+=  "  where ";
            sql+=  "    sodiagnosnum   = ?  ";
