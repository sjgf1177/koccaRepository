...
///////////////////////////////////////////////////////////////////////////////
    ...
    */
    public ArrayList getObjectSulmunResult(DBConnectionManager connMgr, RequestBox box) throws Exception {
		...
		//String v_subj 	= "ALL";			// 일반설문이므로 실 과정코드대신 ALL로 검색.
    ...
    public void ComputeCount(ArrayList p_list, Vector p_answers) throws Exception {
        ...
        try {
            for (int i=0; i < p_answers.size(); i++) {
                ...
                st1 = new StringTokenizer(v_answers,SulmunCommonBean.SPLIT_COMMA, true);
                index = 0;
                cnum =0;
                while ( st1.hasMoreElements() && index < p_list.size()) {
                    ...
                    //==========================================
                    if (v_answer.equals(",")) {
                        if ( (flag) && (st1.hasMoreTokens())) {
                            v_answer = st1.nextToken();

                            if (!v_answer.equals(",") ) cnum++;
                            if (index == 8 && !st1.hasMoreTokens()) lastdata = true;
                        }
                    } else {
                        flag = true;
                        cnum ++;
...
//            sql+= "     and c.f_gubun = " + SQLString.Format(p_gubun);
			...
			System.out.println("SulmunCommonResultBean 일반설문 결과 정답 리스트:"+sql);
            ls = connMgr.executeQuery(sql);
        ...
        try {      
            ...
            if (v_action.equals("go")) {
                ...
                */
                ...
                sql = "select a.userid, b.name, b.sex, substr(b.addr,1,2) addr, resno, ";
                sql+= "       to_char(sysdate,'yyyy') +1 - ( (case  when substr(resno,7,1) < 3 then 19 ";
                sql+= "                                             when substr(resno,7,1) >= 3 then 20 ";
                sql+= "                                       end)  ||  substr(resno,1,2)) age , ";
                sql+= "      b.comp  asgn,  get_compnm(b.comp,2,4) asgnnm,";
                sql+= "	   	  a.subj, a.grcode, a.year, a.subjseq, a.sulpapernum, a.sulnums, a.answers";
                sql+= "	   	  from ";
                sql+= " ( select userid, subj, grcode, year, subjseq, sulpapernum, sulnums, answers ";
                sql+= "  from tz_suleach  ";
                sql+= "   where subj    = 'COMMON'";
