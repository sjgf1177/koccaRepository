//**********************************************************
//  1. 제      목: CMU 게시물 관리
//  2. 프로그램명 : CMUBoardAdminBean.java
//  3. 개      요: CMU 게시물 관리
//  4. 환      경: JDK 1.3
//  5. 버      젼: 1.0공지사항
//  6. 작      성: 이창훈 2005. 7.  14
...
public class CMUBoardAdminBean {

    private ConfigSet config;
    private int row;

    public CMUBoardAdminBean() {
        try {
     ...
     * @throws Exception
     */
    public ArrayList<DataBox> selectCMUBoardList(RequestBox box) throws Exception {
        ...
        try {
            ...
            sql.append("/* com.credu.homepage.CMUBoardAdminBean listCMUBoard (CMU 게시판 목록 조회) */ \n");
            sql.append("SELECT *                                                \n");
            sql.append("  FROM (                                                \n");
            sql.append("        SELECT                                          \n");
            sql.append("                A.SEQ                                   \n");
            ...
            sql.append("          FROM  TZ_BOARD_CMU A                          \n");
     ...
     * @throws Exception
     */
    public int insertCMUBoard(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        ...
        try {
            ...
            /*********************************************************************************************/
            sql.setLength(0);
            sql.append("/* CMUBoardAdminBean insertCMUBoard (CMU 게시물 등록)*/\n");
            sql.append("INSERT  INTO  TZ_BOARD_CMU  (   \n");
            sql.append("        SEQ     \n");
            sql.append("    ,   TITLE   \n");
            sql.append("    ,   CONT    \n");
            sql.append("    ,   USE_YN  \n");
            ...
            if (resultCnt > 0 && (!newFileName.equals("") || (!newImgName.equals("")))) {
                ...
                sql.setLength(0);
                sql.append("/* CMUBoardAdminBean insertCMUBoard (CMU 게시물 파일 등록)*/\n");
                sql.append("INSERT  INTO  TZ_BOARDFILE_CMU  (   \n");
                sql.append("        SEQ             \n");
                sql.append("    ,   FILE_NUM        \n");
                sql.append("    ,   FILE_NM         \n");
                ...
                sql.append(") VALUES (      \n");
                sql.append("        ? \n");
                sql.append("    ,   (SELECT NVL(MAX(FILE_NUM), 0) + 1 AS FILE_NUM FROM TZ_BOARDFILE_CMU WHERE SEQ = ?) \n");
                sql.append("    ,   ?       \n");
                sql.append("    ,   ?       \n");
                sql.append("    ,   ?       \n");
                sql.append("    ,   ?       \n");
     ...
     * @return
     */
    public DataBox selectCMUBoard(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        ListSet ls = null;
        DataBox dbox = null;
        ...
        try {
            ...
            sql.setLength(0);
            sql.append("/* CMUBoardAdminBean selectCMUBoard (CMU 게시물 내용 조회) */ \n");
            ...
            this.updateCMUBoardViewCount(box);
     ...
     * @throws Exception
     */
    public int deleteCMUBoard(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        PreparedStatement pstmt = null;
        PreparedStatement pstmtForFile = null;
        StringBuffer sql = new StringBuffer();
        ...
        try {
            ...
            if (!saveFileNm.equals("")) {
                ...
                sql.setLength(0);
                sql.append("/* deleteCMUBoard (CMU 첨부파일 정보 삭제)*/ \n");
                sql.append("DELETE      \n");
                sql.append("  FROM  TZ_BOARDFILE_CMU\n");
            ...
            sql.setLength(0);
            sql.append("/* deleteCMUBoard (CMU 게시물 삭제)*/ \n");
            sql.append("DELETE      \n");
            sql.append("  FROM  TZ_BOARD_CMU    \n");
            sql.append(" WHERE  SEQ = ? \n");
     ...
     * @throws Exception
     */
    public int updateCMUBoard(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        PreparedStatement pstmt = null;
        PreparedStatement pstmtForFile = null;
        ...
        try {
            ...
            /*********************************************************************************************/
            sql.setLength(0);
            sql.append("/* updateCMUBoard (CMU 게시물 수정)*/ \n");
            sql.append("UPDATE  TZ_BOARD_CMU    \n");
                ...
                if (!newFileName.equals("") || !newImgName.equals("")) { // 신규 파일이 있는 경우
                    ...
                    if((orgSaveFileNm != null && !orgSaveFileNm.equals("")) || (orgSaveImgNm != null && !orgSaveImgNm.equals(""))){
                        ...
                        if(!newFileName.equals("") && !newImgName.equals("")){
                            ...
                            sql.append("UPDATE  TZ_BOARDFILE_CMU    \n");
                        ...
                        sql.append(") VALUES (      \n");
                        sql.append("        ? \n");
                        sql.append("    ,   (SELECT NVL(MAX(FILE_NM), 0) + 1 AS FILE_NUM FROM TZ_BOARDFILE_CMU WHERE SEQ = ? ) \n");
                        sql.append("    ,   ?   \n");
                        sql.append("    ,   ?   \n");
                        sql.append("    ,   ?   \n");
                        sql.append("    ,   ?   \n");
     ...
     * @throws Exception
     */
    public void updateCMUBoardViewCount(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        PreparedStatement pstmt = null;
        StringBuffer sql = new StringBuffer();
        int resultCnt = 0;
        ...
        try {
            if (userAuthority.equals("") || userAuthority.equals("ZZ")) {
                ...
                sql.setLength(0);
                sql.append("/* updateCMUBoard (CMU 게시물 조회수 갱신)*/ \n");
                sql.append("UPDATE  TZ_BOARD_CMU    \n");
