//**********************************************************
//  1. 제      목: 과정 수강생 현황 조회
//  2. 프로그램명 : StudentListBean.java
//  3. 개      요: 과정 수강생 현황 조회
//  4. 환      경: JDK 1.4
//  5. 버      젼: 1.0
//  6. 작      성: 강성욱 2004. 12. 20
//  7. 수      정: 이나연 05.11.22 _ totalcount 하기위한 쿼리수정
//**********************************************************
...
public class StudentListBean {
     ...
     */
    public ArrayList<DataBox> selectListStudent(RequestBox box) throws Exception {
        DBConnectionManager connMgr = null;
        ListSet ls = null;
        ArrayList<DataBox> list = null;
        // 수정일 : 05.11.22 수정자 : 이나연 _ totlacount 하기위한 쿼리수정
        String sql = "";
        String head_sql = "";
        String body_sql = "";
        String group_sql = "";
        ...
        try {
            ...
            head_sql = " select b.userid, ";
            head_sql += "        b.cono, ";
            head_sql += "        b.name, ";
            //            sql += "        get_deptnm(b.deptnam, b.userid)  compnm, ";//부서
            head_sql += "        get_deptnm(b.deptnam, b.userid)  compnm, ";//부서
            head_sql += "        b.email ";
            body_sql += "   from TZ_STUDENT a, ";
            body_sql += "        TZ_MEMBER b ";
            body_sql += "  where a.userid=b.userid ";
            body_sql += "    and a.year='" + v_year + "'";
            body_sql += "    and a.subj='" + v_subj + "'";
            body_sql += "    and a.subjseq='" + v_subjseq + "'";
            if (!v_usernm.equals("")) {
                body_sql += " and b.name like '%" + v_usernm + "%'";
            }
    ...
    //         ls = connMgr.executeQuery(sql);
    //         System.out.println("sql=강나리="+sql);
    //         if (ls.next()){
    //         	v_cnt = ls.getInt(1);
    //         }
    //         ls.close();	
