...
//	2. 프로그램명 : MemberInfoServlet.java
...
@WebServlet("/servlet/controller.homepage.MemberInfoServlet")
public class MemberInfoServlet extends javax.servlet.http.HttpServlet implements Serializable {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException, java.io.IOException {
		...
		try {
			...
			} else if(v_process.equals("memberUpdateNew")) {                  // 수정
				this.performMemberUpdateNew(request, response, box, out);
			} else if(v_process.equals("memberUpdatePageNew")) {                  // 수정페이지로 이동 - 신규
				this.performMemberUpdatePageNew(request, response, box, out);
			} else {
   ...
   @return void
	...
	public void performMemberUpdatePage(HttpServletRequest request, HttpServletResponse response, RequestBox box, PrintWriter out) throws Exception {
		try {
			...
			RequestDispatcher rd = sc.getRequestDispatcher("/learn/user/game/member/gu_MemberInfo.jsp");
   ...
   @return void
	 */
	public void performMemberUpdatePageNew(HttpServletRequest request, HttpServletResponse response, RequestBox box, PrintWriter out) throws Exception {
		try {
			request.setAttribute("requestbox", box);        // 명시적으로 box 객체를 넘겨준다
			...
			ServletContext sc = getServletContext();
			RequestDispatcher rd = sc.getRequestDispatcher("/learn/user/portal/account/zu_privacy_R.jsp");
			rd.forward(request, response);
		}catch (Exception ex) {
			ErrorManager.getErrorStackTrace(ex, out);
			throw new Exception("performLoginPage()\r\n" + ex.getMessage());
    ...
    @return void
	 */
	public void performMemberUpdateNew(HttpServletRequest request, HttpServletResponse response, RequestBox box, PrintWriter out) throws Exception {
		try {
			MemberInfoBean bean = new MemberInfoBean();

			int isOk = bean.memberInfoUpdateNew(box);
