//**********************************************************
//  1. 제      목: 베타테스트시스템 로그인 제어 서블릿
//  2. 프로그램명 : BetaLoginServlet.java
//  3. 개      요: 로그인 제어 프로그램
//  4. 환      경: JDK 1.3
//  5. 버      젼: 1.0
//  6. 작      성: 황창근 2004. 12.  14
//  7. 수     정1:
//**********************************************************
...
import com.credu.beta.BetaLoginBean;
...
@WebServlet("/servlet/controller.beta.BetaLoginServlet")
public class BetaLoginServlet extends javax.servlet.http.HttpServlet implements Serializable {
    ...
    */
    public void performLogin(HttpServletRequest request, HttpServletResponse response, RequestBox box, PrintWriter out) throws Exception {
        try {
            ...
            String v_url = "/beta/index.jsp";
            box.put("p_process", "");
            
            String v_userip = request.getRemoteAddr();
            box.put("p_userip", v_userip);
            
            BetaLoginBean bean = new BetaLoginBean();
...
//            if (isOk == 1) {
                ...
                if(isOk1 > 0) {
                    ...
                    //isOk3 = MileageManager.insertMileage(v_code, s_userid);    // 마일리지 작성
                    ...
                    if (v_rememberId.equals("1")) {
                        ...
                        Cookie useridCookie = new Cookie("userid_beta",v_userid);
                        useridCookie.setMaxAge(60*60*24*365);
                        response.addCookie(useridCookie);
                    } else {
            
                        Cookie useridCookie = new Cookie("userid_beta","");
                ...
                if (v_frmURL.equals("")) {
                	if (isOk1 > 0) {
                		...
                		Log.info.println(this, box, v_msg + " on BetaLoginServlet");
                		alert.alertOkMessage(out, v_msg, v_url , box);
                	}
                } else {
   ...
   */
   public void performLoginPage(HttpServletRequest request, HttpServletResponse response, RequestBox box, PrintWriter out) throws Exception {
       try {
           ...
           RequestDispatcher rd = sc.getRequestDispatcher("/beta/index.jsp");
           rd.forward(request, response);

            Log.info.println(this, box, "Dispatch to /beta/index.jsp");
