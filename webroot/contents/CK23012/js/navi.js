...
var chapter = nowIndex.substring(nowIndex.length-6,nowIndex.length-4); // 차시
var page = nowIndex.substring(nowIndex.length-2,nowIndex.length); // 페이지
var subjectCode = "CK23010"; // 과정코드
...
function next(n){
	...
	else
	{
		...
		location.href = prefix_page_move + itostr(next_page) + File_makeName;
	}
}
...
function prev(n){
	...
	else
	{
		...
		location.href = prefix_page_move + itostr(prev_page) + File_makeName;
	}
}
