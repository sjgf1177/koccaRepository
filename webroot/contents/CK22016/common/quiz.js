...
//		if(this.isIncorrect && selectedQuiz < incorrectQuiz.length){
	...
	this.checkAnswer = function(){
		...
		if(selectedAnswer[selectedQuiz]+1 == this.Quiz[currentChapter].questions[selectedQuiz].answer){
			audioEnd();
	...
	this.settingDap = function() {
		...
		$("#dapNumber").append("<span>정답<div>"+ ansStr+"</div></span>");
		/*
...
//		$("DIV.select ul li").addClass("notselected");
	...
	this.result = function(){
		...
		for ( var i = 0; i < this.Quiz[currentChapter].questions.length; i++) {
			...
			if(selectedAnswer[i]+1 == this.Quiz[currentChapter].questions[i].answer){
				correctCount++;
		...
		*/
		...
		if( this.Quiz[currentChapter].questions[selectedQuiz].qztype!="OX"){
			for(var i=0; i < choice.length; i++){
				$("#qsel").css('position','absolute').css('top','195px');
