...
function sInit(){
	...
	var _surl;
	pageName=itostr(currentChapter)+itostr(currentClauPage);
	pageNameNew = "KPC_2022_kawf_01_"+pageName;

	//_surl="../mp4/"+pageName+".mp4";
	

	if(currentClau!=movieClau){
			var surlname=menuNaviNameSet[currentChapter][currentPage];
		_surl="http://edu_kocca.ecn.cdn.infralab.net/kocca/mp4/onlineclass/CK22016/mp4/"+itostr(currentChapter)+surlname.toLowerCase()+".mp4";
	}else{
		
		_surl="http://edu_kocca.ecn.cdn.infralab.net/kocca/mp4/onlineclass/CK22016/mp4/"+itostr(currentChapter)+itostr(currentClauPage)+".mp4";
	
	}

	if(loadFunction){
	lp = Number(getCookie('savePoint_' + pageNameNew));
	if(lp){
	loadPoint=lp;
	}else{
	loadPoint=0;
	...
	cp=getCookie('isComplete_' + pageNameNew);
	if(cp=="Y"){
	isComplete="Y"
	}else{
	isComplete="N";
...
if(loadFunction){
	if(isComplete=="N"){
		if(!(loadPoint==0 || loadPoint==null || typeof(loadPoint) == 'undefined')){
		var result = confirm("이전에 학습한 이력이 있습니다. 이어보기 하시겠습니까?");
		if(result){		  
			  _video.addEventListener('playing',_playing,false);
			 // lchkID = setInterval( loadTime, 100)
			 lchkID =  setInterval(function(){loadTime(loadPoint)}, 10);
			}else{
			_video.addEventListener('playing',_playing,false);
			}
		}
			...
			if(bType.indexOf("Chrome")!=-1){
				alert("크롬 브라우저의 경우 학습종료시 학습창 상단 오른쪽 검정색 닫기 버튼을 눌러야 학습 시점이 저장됩니다.");
			}
			*/

	}else{
		alert("해당 페이지 학습을 완료하였습니다.");
		globalDrag=true;
		_video.addEventListener('playing',_playing,false);
	}
//}
...
function loadTime(mvtime){
	if(_mvcurTime>=0 &&  _mvcurTime!=null){
		_video.pause();
	//alert("학습한 포인트로 이동합니다.:"+mvtime);	
	clearInterval(lchkID);
	 audioJump(mvtime);
	}
}
...
function _slideclick(e){
	console.log("globalDrag : " + globalDrag);
	console.log("isPageControl : " + parent.isPageControl);
		if(globalDrag){
	//isDraging=true;
	var xx = e.clientX-_dragX-_dragGap;
	if(e.clientX<_dragX){
...
//		_timeTxt.innerHTML = rectime(cTime)+" / "+rectime(tTime);
	...
	if(cTime > tTime - 0.1) {
		if(typeof(dontShowPopupChapterByVideoEnd) == 'undefined' || dontShowPopupChapterByVideoEnd == false){
			...
			globalDrag=true;
			if(loadFunction){
			setCookie('isComplete_' + pageNameNew,"Y",365);
			//////setComplete(pageNameNew, _mvcurTime);
			}
	...
	// 비디오재생에 따라 액션이 이루어져야 할때 호출하는 함수
...
function _fullscreenMv(e){
	...
	if(!isFullScreen){
		...
		//	$("#controlset").css("display","inline-block");
			...
			//$('.slidegap').css("margin-left","400px"); // 컨트롤간격
...
function offFullscreen(){
		...
		control.style.width="1280px";
			...
			//$('.slidegap').css("margin-left","10px"); // 컨트롤간격
...
function fnSaveEndPoint(){
	//alert("학습시점 저장:"+_mvcurTime);	
	setCookie('savePoint_' + pageNameNew, _mvcurTime,365);
	/* lms */
	var cTime = _video.currentTime;
	var tTime = _video.duration;
	if( cTime != tTime ) {
		//alert("호출!");
		setEndTime(pageNameNew, _mvcurTime);
	}
}
