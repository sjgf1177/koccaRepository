var pageTotal =  Object.keys(summaryInfo).length;
var curPage = 1;
var completeConf = []
...
function  buildSummary(n){
	...
	var html = '';
	for(i = 0; i < summaryInfo[curPage].length; i++){
		var _rank = summaryInfo[curPage][i].rank;
		var _text = summaryInfo[curPage][i].text;
		_text = _text.replaceAll('|','<br/>');
		_text = _text.replaceAll('{','<span class="summary-rank'+_rank+'-txt-impact">');
		_text = _text.replaceAll('}','</span>');
		if(isNaN(_rank)){
			if(_rank == "-blank"){
				html += '<div class="summary-rank'+_rank+'"></div>';
			}else{
				var _num = _text.split('. ');
				html += '<div class="summary-rank'+_rank+'">';
				html += '<div class="summary-rank'+_rank+'-bullet">'+_num[0]+'</div>';
				html += '<div class="summary-rank'+_rank+'-txt">'+_num[1]+'</div>';
				html += '</div>';
			}
		}else{
			if(_rank > 1){
				html += '<div class="summary-rank'+_rank+'">';
				html += '<div class="summary-rank'+_rank+'-bullet"></div>';
				html += '<div class="summary-rank'+_rank+'-txt">'+_text+'</div>';
				html += '</div>';
			}else{
				html += '<div class="summary-rank'+_rank+'-txt">'+_text+'</div>';
			}
