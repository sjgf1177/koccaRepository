...
//		setTimeout("eduChkStart('START')",1000);
            ...
            if (v_subj == 'cpl003' || v_subj == 'cpl004' || v_subj == 'cpl010' || v_subj == 'sws006'
                ...
                if ('<%=s_tem_type%>' == 'B') {
                    if (document.next.now.value == "") {
                        document.next.now.value = 1;
                    }
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        data: {
                            chap: parseInt(document.next.now.value),
                            p_process: "subjPageInfo"
                        },
                        success: function (data) {
                            var res = "";
                            var tmpUrl = urls.split("/");
                            var v_url = "";

                            if (data.resList.length > 0) {
                                res = data.resList[0];
                                if (res.pageChkYn == 'Y' && res.nextPageChkYn == 'Y') {
                                    for (var x = 1; x < tmpUrl.length; x++) {
                                        if (tmpUrl[x].indexOf("html") > -1) {
                                            tmpUrl[x] = res.np;
                                        }
                                        v_url += "/" + tmpUrl[x];
                                    }
        ...
        function starting(lesson) {
            ...
            if (lesson > 1) {
                $.ajax({
                    ...
                    dataType: "json",
                    data: {
                        p_process: "subjSeqPageChk"
                    },
                    success: function (data) {
                        if (data.pageChkYn == 'Y') {
                            lessonCompleteChk(lesson);
                        } else {
                            jumpTo(lesson);
                        }
                    }, error: function (arg1, arg2) {
        ...
        function lessonCompleteChk(lesson) {
            $.ajax({
                type: "post",
                url: "/servlet/controller.contents.EduStart",
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    if (data.resList.length > 0) {
                        res = data.resList[0];
                        alert(parseInt(res.lesson) + "차시 [" + res.sdesc + "] \n학습이 완료되지 않았습니다.");
                    } else {
                        jumpTo(lesson);
                    }
    ...
    <FORM name="sel">
            ...
            <TR>
                ...
                <TD width="*" align="CENTER" valign=middle background="<%=v_imgURL%>/title2.gif">&#160;
                    <SELECT name="p_lesson" onChange="top.etop.eduChk();whenSelect(document.sel.p_lesson.selectedIndex);">
                        ...
                        <option value="<%=x.getLesson()%>,<%=x.getStarting()%>,<%=x.getIsexam()%>,<%=x.getIsbranch()%>"><%=x.getLesson()%>
                            -<%=x.getSdesc()%>
                        </option>
                        <% } %>
                    </SELECT>
        ...
        <TR>
            <td align="right" valign="top"><!-- 2009.12.04 선택상자변경시  학습시간 반영 top.etop.eduChk(); -->
                <SELECT name="p_lesson" style="margin-top:13px;"
                        onChange="top.etop.eduChk();whenSelect(document.sel.p_lesson.selectedIndex);">
                    <% for (int i = 0; i < list.size(); i++) {
                        x = (MfLessonData) list.get(i); %>
                    <option style="font-family:'gulim','dotum','sans-serif'; font-size:12px;'"
                            value="<%=x.getLesson()%>,<%=x.getStarting()%>,<%=x.getIsexam()%>,<%=x.getIsbranch()%>"><%=x.getLesson()%>
                        -<%=x.getSdesc()%>
                    </option>
                    <% } %>
                </SELECT>
